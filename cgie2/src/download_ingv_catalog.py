...
1. Apri il repository cgie-global-instability-radar.

2. Apri la cartella:

   cgie2

3. Crea o apri la cartella:
...
5. Come nome del file inserisci:

   download_ingv_catalog.py

6. Incolla tutto il contenuto Python compreso tra:
...
7. Nel campo Commit message inserisci:

   Add frozen INGV catalogue downloader for CF-RETRO-01

8. Nel campo Description inserisci:

   Adds the acquisition script for CF-RETRO-01. The script reads the
   frozen YAML configuration, downloads the INGV earthquake catalogue in
   time chunks, applies the fixed 15 km geographic boundary and the
   pre-event cutoff, validates and deduplicates the records, and writes
   the frozen catalogue with a reproducibility manifest and SHA-256 hash.
...
Il prossimo commit creerà:

requirements-cgie2.txt

Servirà a installare e congelare le librerie Python richieste prima
