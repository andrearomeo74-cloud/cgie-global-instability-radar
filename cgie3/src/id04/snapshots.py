...
def build_scale_snapshots(
    ...
    *,
    ...
    if first_endpoint_position >= len(
        timestamps
    ):
        fail(
            "No admissible snapshot endpoints exist for "
    ...
    for snapshot_index, endpoint_position in enumerate(
        ...
        if (
            ...
            or snapshot_index == endpoint_count
        ):
            print(
                f"[ID04] scale={scale_id} "
                f"snapshot={snapshot_index}/{endpoint_count} "
        ...
        for (
            ...
            estimate = _spearman_from_arrays(
                feature_arrays[
                    source_id
                ][
                    left_position:
